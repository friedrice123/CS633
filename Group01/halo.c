...
int main(int argc, char *argv[]) {
    ...
    for (int step = 0; step < num_time_steps; step++) {
        ...
        if(has_top && !has_left){
            ...
            MPI_Isend(top_buff_recv, Px*position, MPI_PACKED, rank - Px, rank, MPI_COMM_WORLD, &requests1[request_count1++]);
        ...
        if(has_bottom && !has_left){
            ...
            // MPI_Wait(&requests[request_count1-1], &status[status_count-1]);   // Waiting for the communication to complete
        ...
        if(has_bottom && !has_left){
            MPI_Isend(bottom_buff_recv, Px*position, MPI_PACKED, rank + Px, rank, MPI_COMM_WORLD, &requests1[request_count1++]);  
        }
